...
-- 2. find id and app name for all apps that were last updated on aug 1
...
-- 5. Find app with most reviews thats has rating greater than 4.8
SELECT * FROM analytics 
  WHERE rating >= 4.8 
  ORDER BY reviews DESC
  LIMIT 1;

-- 6. Find the average rating ordered by the highest rated to lowest rated.
SELECT category, AVG(rating) FROM analytics 
  GROUP BY category 
  ORDER BY avg DESC;

-- 7. Find the name, price, and rating of the most expensive app with a rating 
-- that's less than 3
...
-- 8. min install not exceeding 50, that have a rating. results by highest rated first
SELECT * FROM analytics 
  WHERE min_installs <= 50 
    AND rating IS NOT NULL 
  ORDER BY rating DESC;

-- 9. apps that are rated less than 3 with at least 10000 reviews.
